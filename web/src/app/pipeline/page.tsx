...
import { CUSTOMERS, LEADS, STAGES, customerName, type Lead } from "@/lib/os-demo";
import { useDialer } from "@/components/os/DialerDock";

export default function PipelinePage() {
  const { dial } = useDialer();
  const [rows, setRows] = useState<Lead[]>(LEADS.map((l) => ({ ...l })));
  const [filter, setFilter] = useState("all");
  const shown = useMemo(() => rows.filter((l) => filter === "all" || l.cust === filter), [rows, filter]);
